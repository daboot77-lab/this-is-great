...
# 외부 라이브러리 없이 순수 HTML Canvas 및 CSS Animation 타임라인으로 깜빡임 없는 명품 구도를 완성합니다.
# 사용자가 소원을 입력하면, 브라우저 스토리지 및 세션 상태에 완전 보존됩니다.
...
<head>
    ...
    <style>
        ...
        .earth-ball {
            font-size: 42px;
            line-height: 1;
            filter: drop-shadow(0 0 10px rgba(34, 197,  green, 0.3));
...
<div class="universe-container" id="universe">
    ...
    <div class="star-character" id="altair">
        ...
        <div class="clothing-badge" style="color: #a5c5ff; border-color: rgba(165,197,255,0.3);">견우 (고구려 바지)</div>
    </div>
    ...
    <div class="star-character" id="vega">
        ...
        <span class="avatar-emoji">👸</span>
        <div class="clothing-badge" style="color: #ffaef8; border-color: rgba(255,174,248,0.3);">직녀 (주름치마)</div>
    </div>
...
<script>
    ...
    function transitionToSpringAutumn() {
        ...
        // 견우, 직녀성 지평선 부근(낮음 - top: 280px)에 배치하고 착시로 인해 좌우 끝으로 가장 넓게 벌림(left: 10%, right: 10%)
        ...
        document.getElementById("console").innerHTML = `
            <div style="font-weight: bold; color: #ff9999; margin-bottom: 2px;">🌌 1단계: 봄/가을 (낮은 고도와 평평화 착시)</div>
            <div>${narrativeData.spring_autumn}</div>
            <div class="sub-dialogue-badge">${dialogueData.spring_autumn}</div>
        `;
        speakKorean(narrativeData.spring_autumn + " " + dialogueData.spring_autumn);

        // 2단계: 여름/칠월칠석 자동 전환 (4.5초 후 자동 이동)
        timelineTimer = setTimeout(() => {
    ...
    function transitionToChilseok() {
        ...
        // 견우, 직녀성이 중천 꼭대기(높음 - top: 75px)로 이동하며, 착시가 제거되어 가깝게 좁아짐(left: 31%, right: 31%)
        const altair = document.getElementById("altair");
        ...
        // 콘솔 및 대화 자막 업데이트 및 TTS 출력
        document.getElementById("console").innerHTML = `
            <div style="font-weight: bold; color: #a3e2ff; margin-bottom: 2px;">🌌 2단계: 여름 칠월칠석 (머리 위 중천 고도 이동)</div>
            <div>${narrativeData.chilseok}</div>
            <div class="sub-dialogue-badge">${dialogueData.chilseok}</div>
        `;
        speakKorean(narrativeData.chilseok + " " + dialogueData.chilseok);

        // 3단계: 오작교 결합 자동 전환 (4.5초 후 자동 설치)
        timelineTimer = setTimeout(() => {
            transitionToBridge();
        }, 6500);
    }
    ...
    function transitionToBridge() {
        ...
        // 두 캐릭터가 오작교 다리 위 한가운데서 완전히 조우함 (수평 41% 밀착)
        const altair = document.getElementById("altair");
        ...
        // 콘솔 및 대화 업데이트, TTS 출력
        document.getElementById("console").innerHTML = `
            <div style="font-weight: bold; color: #a3ffb1; margin-bottom: 2px;">🌉 3단계: 까마귀와 까치의 오작교 재회</div>
            <div>${narrativeData.bridge}</div>
            <div class="sub-dialogue-badge">${dialogueData.bridge}</div>
        `;
        speakKorean(narrativeData.bridge + " " + dialogueData.bridge);

        // 4단계: 소원 편지 배달통 하강 (4.5초 후 자동 작동)
        timelineTimer = setTimeout(() => {
            transitionToWish();
        }, 6500);
    }
    ...
    function transitionToWish() {
        ...
        // 소원 편지봉투가 지구 관측자(사용자) 머리맡 위로 사락 내려옴
        document.getElementById("wishEnvelope").classList.add("delivered");

        // 콘솔 메시지 갱신
        document.getElementById("console").innerHTML = `
            <div style="font-weight: bold; color: #ffd700; margin-bottom: 2px;">✉️ 4단계: 축복의 은하수 소원 편지 배달</div>
            <div>${narrativeData.wish}</div>
        `;
    ...
    function submitWish() {
        ...
        // 콘솔 완료 알림 및 TTS
        document.getElementById("console").innerHTML = `
            <div style="font-weight: bold; color: #ffd700; margin-bottom: 2px;">🌟 최종 완료: 은하수 소원 별 등극!</div>
            <div>당신의 소망이 은하수 하늘 높이 전송되어 영원히 꺼지지 않는 소원 별로 환하게 수놓아졌습니다!</div>
            <button class="btn-reset" onclick="resetSimulator()">시뮬레이터 초기화 🔄</button>
        `;
        speakKorean("당신의 소중한 기도가 은하수 하늘 높이 전송되어 찬란히 빛나는 소원 별이 되었습니다.");

        // 부모 창(Streamlit)에 소원 등록 완료 신호 전송하여, Streamlit의 보관함 및 F5 영속성을 온전히 연계
        if (window.parent) {
    ...
    function resetSimulator() {
        ...
        document.getElementById("console").innerHTML = `
            <div style="font-weight: bold; color: #ffd700; margin-bottom: 2px;">🌌 관측 대기 콘솔</div>
            <span>가운데 지구(🌍) 캐릭터를 클릭하여 지구의 공전을 가동하고 천문 설화 애니메이션을 자동 재생시키세요.</span>
        `;
...
# HTML 컴포넌트를 사용해 캔버스를 화면 정중앙에 고도로 정교하게 임베드
# 부모 파이썬 Streamlit이 postMessage 데이터를 받아 st.session_state에 완벽하게 영속 기록할 수 있도록 설계
components_output = components.html(timeline_html_code, height=450)

# 만약 자바스크립트 측에서 postMessage를 통해 새로운 소원 리스트를 반환했다면 세션 데이터 업데이트
